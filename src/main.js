import Vue from 'vue';
import App from './App.vue';
import router from './router';
import store from './store';
import './style/main.scss';
import './style/global.less';
import 'vant/lib/index.css';
import {Cell, DatetimePicker, Popup} from 'vant';
import '@/plugins/element-ui';
import _ from 'lodash';

import httpConfig from './http/index';

Vue.config.productionTip = false;

Vue.use(httpConfig);
Vue.use(Cell);
Vue.use(Popup);
Vue.use(DatetimePicker);
Vue.use(_);

let instance = null;

/**
 * 渲染函数
 * 两种情况：主应用生命周期钩子中运行 / 微应用单独启动时运行
 */
function render(props = {}) {
    const {container} = props;
    instance = new Vue({
        router,
        store,
        render: (h) => h(App),
    }).$mount(container ? container.querySelector('#app') : '#app');
}

/**
 * 独立运行时，直接挂载应用
 * 如果是独立运行window.__POWERED_BY_QIANKUN__= undefined
 **/
if (!window.__POWERED_BY_QIANKUN__) {
    render()
}

/**
 * bootstrap 只会在微应用初始化的时候调用一次，下次微应用重新进入时会直接调用 mount 钩子，不会再重复触发 bootstrap。
 * 通常我们可以在这里做一些全局变量的初始化，比如不会在 unmount 阶段被销毁的应用级别的缓存等。
 */
export async function bootstrap() {
    console.log("anomalyDetectionManageApp bootstraped");
}

/**
 * 应用每次进入都会调用 mount 方法，通常我们在这里触发应用的渲染方法
 */
export async function mount(props) {
    console.log("anomalyDetectionManageApp mount", props);
    const {container} = props;
    proxy(container);
    render(props);
}

// 增加 update 钩子以便主应用手动更新微应用
export async function update(props) {
    const {container} = props;
    proxy(container);
    render(props);
}

/**
 * 应用每次 切出/卸载 会调用的方法，通常在这里我们会卸载微应用的应用实例
 */
export async function unmount() {
    console.log("anomalyDetectionManageApp unmount");
    instance.$destroy();
    instance.$el.innerHTML = '';
    instance = null;
}

/**
 * 子应用加载时代理 document.body.appendChild
 */
function proxy(shadowDom) {
    document.body.appendChild = new Proxy(document.body.appendChild, {
        apply(target, thisArg, node) {
            // console.log(target, thisArg, node);
            if (node[0].classList.contains('el-overlay')) {
                target.apply(thisArg, node);
            } else {
                shadowDom.appendChild(node[0]);
            }
        },
    });
}
